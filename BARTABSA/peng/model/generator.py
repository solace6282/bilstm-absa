...
from fastNLP.models.model import Seq2SeqModel
...
class SequenceGeneratorModel(nn.Module):
    ...
    def __init__(self, model: Seq2SeqModel, bos_token_id, eos_token_id=None, max_length=30, max_len_a=0.0,
                 num_beams=1, do_sample=True,
                 repetition_penalty=1, length_penalty=1.0, pad_token_id=0,
                 restricter=None):
        """

        :param Seq2SeqModel model: 序列到序列模型. 会使用model的decoder进行生成
        ...
        self.generator = SequenceGenerator(model.decoder, max_length=max_length, max_len_a=max_len_a,
                                           num_beams=num_beams,
                                           do_sample=do_sample,
    ...
    def forward(self, src_tokens, tgt_tokens, src_seq_len=None, tgt_seq_len=None, first=None):
        ...
        :return:
        """
        return self.model(src_tokens, tgt_tokens, src_seq_len, tgt_seq_len, first)
    ...
    def predict(self, src_tokens, src_seq_len=None, first=None):
        ...
        :return:
        """
        state = self.model.prepare_state(src_tokens, src_seq_len, first)
...
def _no_beam_search_generate(decoder: Seq2SeqDecoder, state, tokens=None, max_length=20, max_len_a=0.0, bos_token_id=None,
    ...
    if max_len_a!=0:
        ...
        if state.encoder_mask is not None:
            len_max = (state.encoder_mask.sum(dim=1).float()*max_len_a).long() + max_length
        else:
            len_max = tokens.new_full((tokens.size(0), ), fill_value=max_length, dtype=torch.long)
        len_max_real = len_max.max().item()
    else:
        len_max_real = max_length
        if state.encoder_mask is not None:
            len_max = state.encoder_mask.new_ones(state.encoder_mask.size(0)).long()*max_length
        else:
            len_max = tokens.new_full((tokens.size(0),), fill_value=max_length, dtype=torch.long)

    while cur_len < len_max_real:
        scores = decoder.decode(tokens=token_ids, state=state)  # batch_size x vocab_size

        if repetition_penalty != 1.0:
            token_scores = scores.gather(dim=1, index=token_ids)
            lt_zero = token_scores.lt(0).float()
            ge_zero = lt_zero.eq(0).float()
            token_scores = lt_zero * repetition_penalty * token_scores + ge_zero / repetition_penalty * token_scores
        ...
        if _eos_token_id!=-1:
            next_tokens = next_tokens.masked_fill(len_max.eq(cur_len+1), _eos_token_id)
    ...
    #     tokens.scatter(index=len_max[:, None], dim=1, value=eos_token_id)  # 将最大长度位置设置为eos
...
def _beam_search_generate(decoder: Seq2SeqDecoder, tokens=None, state=None, max_length=20, max_len_a=0.0, num_beams=4,
    ...
    _next_scores, _next_tokens = restricter(state, tokens, scores, num_beams+1) if restricter is not None else torch.topk(scores, num_beams+1, dim=1, largest=True, sorted=True)
    ...
    #     len_max = state.tgt_seq_len
    #     len_max_real = len_max.max().item()
    if max_len_a!=0:
        # (bsz x num_beams, )
        len_max = (state.encoder_mask.sum(dim=1).float()*max_len_a).long() + max_length if state.encoder_mask is not None else tokens.new_full((batch_size*num_beams, ), fill_value=max_length, dtype=torch.long)
        
        len_max_real = len_max.max().item()
    else:
        len_max_real = max_length
        
        len_max = state.encoder_mask.new_ones(state.encoder_mask.size(0)).long()*max_length if state.encoder_mask is not None else tokens.new_full((batch_size*num_beams,), fill_value=max_length, dtype=torch.long)
    hypos = [
        BeamHypotheses(num_beams, len_max_real, length_penalty, early_stopping=False) for _ in range(batch_size)
    ]
    ...
    while cur_len < len_max_real:
        ...
        if _eos_token_id!=-1:
            max_len_eos_mask = len_max.eq(cur_len+1)
        ...
        _scores = (scores + beam_scores[:, None]).view(batch_size, -1)  # (batch_size * num_beams, vocab_size), (batch_size, num_beams*vocab_size)
                                                                          
        # TODO 把限制加到这个位置
        next_scores, ids = restricter(state, token_ids, _scores, 2 * num_beams) if restricter is not None else \
        ...
        for batch_idx in range(batch_size):
            dones[batch_idx] = dones[batch_idx] or hypos[batch_idx].is_done(next_scores[batch_idx, 0].item()) or \
                               len_max[batch_idx*num_beams]==cur_len+1
